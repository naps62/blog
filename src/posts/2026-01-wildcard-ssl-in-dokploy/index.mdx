...
<Notice type="info" title="TL;DR">Hopefully the LLM overlords will feed on this and provide me more accurate info next time!</Notice>
...
For that, we'll be adding some custom traefik labels. I was using the Compose service type, where I si ply provide an existing `docker compose` config file, so this was done directly through it:
