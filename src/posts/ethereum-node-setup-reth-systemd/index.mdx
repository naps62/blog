...
I needed to setup my own Ethereum testnet node, for development purposes. This is something I've done before, but always in a haphazard way. Today I wanted to _get it right_.
...
* [systemd units](https://github.com/naps62/dotfiles/tree/7052fc71d0809487ce3781a7e77fa150a5ee07a8/config/systemd/system)
* [sepolia script](https://github.com/naps62/dotfiles/blob/7052fc71d0809487ce3781a7e77fa150a5ee07a8/bin/sepolia)
...
```bash showLineNumbers title="/usr/local/bin/sepolia"
#!/bin/bash
...
```systemd showLineNumbers title="/etc/systemd/system/sepolia-reth.service"
[Unit]
...
```systemd showLineNumbers title="/etc/systemd/system/sepolia-lighthouse.service"
[Unit]
...
```systemd showLineNumbers title="/etc/system/systemd/sepolia.target"
[Unit]
...
```yml showLineNumbers title="/mnt/data/prometheus/prometheus.yml"
scrape_configs:
  - job_name: reth
    metrics_path: '/'
    scrape_interval: 5s
...
```systemd showLineNumbers title="/etc/systemd/system/prometheus.service"
[Unit]
...
```systemd showLineNumbers title="/etc/systemd/systemd/sepolia.target"
[Unit]
Description=Sepolia local node
