...
images: [/staatic/images/posts/ethereum-node-setup-reth-systemd/banner.png]
---
...
```systemd:/etc/systemd/system/sepolia-reth.service
[Unit]
...
```systemd:/etc/systemd/system/sepolia-lighthouse.service
[Unit]
...
```systemd:/etc/system/systemd/sepolia.target
[Unit]
...
```yml:/mnt/data/prometheus/prometheus.yml
scrape_configs:
  - job_name: reth
    metrics_path: '/'
    scrape_interval: 5s
...
```systemd:/etc/systemd/systemd/sepolia.target
[Unit]
Description=Sepolia local node
...
![grafana](/static/images/posts/ethereum-node-setup-reth-systemd/grafana.png)
